 ...
 * @tbl is the user's table string $email is the user's email to be checked
 */
    
function is_user_registered($tbl,$email) {

   $row = DB::query('SELECT * FROM %l WHERE email =%s',$tbl,$email);
...
function in_db($table, $column, $itemn) {
    ...
    $row = DB::queryFirstRow("SELECT * FROM %l WHERE %l = %s",$table, $column, $itemn);
...
function in_db_int($table, $column, $itemn) {
    
    $row = DB::queryFirstRow("SELECT * FROM %l WHERE %l = %i",$table, $column, $itemn);
...
function get_states(){
    ...
    $st = "<option value=\"none\">Select ...</option";
...
function bank_in_db($owner, $number){
    $row =DB::query("SELECT * FROM bank WHERE owner = %s AND number =%s",$owner,$number);
if(count($row) >0){
    return TRUE;
}
return false;
